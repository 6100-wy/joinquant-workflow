...
2. **签到**：找“签到”按钮；若显示“已签到”则跳过，否则点击领取 1 积分/天。
   - **验证码兜底（重要）**：点击后若弹出滑块/对图案等验证码，**停止自动操作**，保留可见浏览器窗口，提示用户在窗口内手动完成（拖滑块/点图案，1-2 秒），等待用户回复“好了”后再继续。**禁止反复自动重试**（会触发风控）。若本次会话无可见窗口，改用 `--headed` 重启后让用户完成。
3. **浏览社区文章**：每浏览 1 篇文章自动得 1 积分（上限 30 分/月）；若按钮显示“已完成”则本月已达标，否则打开社区文章列表逐篇浏览（每篇停留 2-3 秒）直至达标或达到本次计划篇数。
4. 完成后可选读取“可用积分”确认。
