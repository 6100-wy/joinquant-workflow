...
3. **浏览社区文章**：每浏览 1 篇文章自动得 1 积分（上限 30 分/月）；若按钮显示“已完成”则本月已达标，否则打开社区文章列表逐篇浏览（每篇停留 2-3 秒）。
   - **验证码（2026-08-15 确认）**：浏览同样会触发“拉动图标/拼图验证”（与签到同类），AI 无法自动通过。出现验证码时停止自动操作，让用户在可见窗口手动完成后再继续下一篇；未完成验证码时积分不会到账（08-15 实测浏览 3 篇无积分变化）。
4. 完成后可选读取“可用积分”确认。
